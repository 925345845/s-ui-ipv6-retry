...
export default {
  ...
  computed: {
    ...
    optionPinned: {
      get(): boolean { return this.outTls.pinned_peer_certificate_sha256 != undefined },
      set(v:boolean) { this.tls.client.pinned_peer_certificate_sha256 = v ? [] : undefined }
    },
    pinnedSha256Text: {
      get(): string { return this.outTls.pinned_peer_certificate_sha256 ? this.outTls.pinned_peer_certificate_sha256.join('\n') : '' },
      set(v:string) { this.tls.client.pinned_peer_certificate_sha256 = v.split('\n').map((s:string)=>s.trim()).filter((s:string)=>s.length>0) }
    },
    optionTime: {
