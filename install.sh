...
download_release() {
    ...
    local arch_name
    arch_name="$(arch)"
    local url="https://github.com/925345845/s-ui-ipv6-retry/releases/download/${version}/s-ui-linux-${arch_name}.tar.gz"
...
install_s-ui() {
    ...
    if [[ -z "$REQUESTED_VERSION" ]]; then
        last_version=$(curl -Ls "https://api.github.com/repos/925345845/s-ui-ipv6-retry/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            last_version=$(curl -Ls "https://api.github.com/repos/925345845/s-ui-ipv6-retry/releases?per_page=5" | grep '"tag_name":' | head -1 | sed -E 's/.*"([^"]+)".*/\1/')
        fi
        if [[ ! -n "$last_version" ]]; then
    ...
    if [[ "$INSTALL_KIND" == "minimal" ]]; then
        ...
        echo -e "  bash <(curl -Ls https://raw.githubusercontent.com/925345845/s-ui-ipv6-retry/main/install.sh) -y --full --domain 你的域名"
    fi
    if [[ "$INSTALL_KIND" == "managed" ]]; then
