...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/925345845/s-ui-ipv6-retry/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/925345845/s-ui-ipv6-retry/main/install.sh"
